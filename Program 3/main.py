...
# print(sports_text)
education_text1 = scrape_wikipedia(education_url)
education_text2 = scrape_wikipedia(education_url)
education_text = education_text1 + education_text2

# Preprocess and clean text
doc1 = preprocess_text(sports_text)
doc2 = preprocess_text(education_text)
# write this into a file

# Write the cleaned text into files
unigram_vectorizer = CountVectorizer(
    ngram_range=(1, 1), token_pattern=r"\b\w+\b", min_df=1
)
X_unigram_counts = unigram_vectorizer.fit_transform([doc1, doc2])
...
bigram_vectorizer = CountVectorizer(
    ngram_range=(2, 2), token_pattern=r"\b\w+\b", min_df=1
)
X_bigram_counts = bigram_vectorizer.fit_transform([doc1, doc2])
...
for i in range(X_bigram_counts.shape[0]):
    for j in range(X_bigram_counts.shape[1]):
        ...
        if unigram_count1 == 0 or unigram_count2 == 0:
            ...
            X_bigram_probs[i, j] = X_bigram_counts[i, j] / min(
                unigram_count1, unigram_count2
            )
...
X_test_counts = unigram_vectorizer.transform([doc1, doc2])
X_test_bigram_counts = bigram_vectorizer.transform([doc1, doc2])
...
for i in range(X_test_bigram_counts.shape[0]):
    for j in range(X_test_bigram_counts.shape[1]):
        ...
        if unigram_count1 == 0 or unigram_count2 == 0:
            ...
            X_test_bigram_probs[i, j] = X_test_bigram_counts[i, j] / min(
                unigram_count1, unigram_count2
            )

X_test_tfidf = tfidf_vectorizer.transform([doc1, doc2])


# use naive bayes classifier for each matrix

X_train, X_test, y_train, y_test = train_test_split(
    X_unigram_counts, [0, 1], test_size=0.5, random_state=0
)
...
X_train, X_test, y_train, y_test = train_test_split(
    X_bigram_counts, [0, 1], test_size=0.5, random_state=0
)
...
X_train, X_test, y_train, y_test = train_test_split(
    X_bigram_probs, [0, 1], test_size=0.5, random_state=0
)
