...
def remove_special_character(
    input_text: str, special_characters: Optional[str] = None
) -> str:
...
def normalize_unicode(input_text: str) -> str:
    ...
    processed_tokens = (
        normalize("NFKD", input_text).encode("ASCII", "ignore").decode("utf8")
    )
...
def remove_stopword(
    input_text_or_list: Union[str, List[str]], stop_words: Optional[set] = None
) -> List[str]:
    """Remove stop words"""
...
def remove_credit_card_number(input_text: str) -> str:
    ...
    regex_pattern = (
        "(4[0-9]{12}(?:[0-9]{3})?|(?:5[1-5][0-9]{2}|222[1-9]|22[3-9][0-9]|2[3-6][0-9]{2}|27[01]["
...
def remove_html_tags(input_text: str) -> str:
    ...
    # if <h1>ans</h1> is present in the input text, it will be parsed as "and"
    return re.sub("<[^<]+?>", "", input_text)


def preprocess_text(
    input_text: str, processing_function_list: Optional[List[Callable]] = None
) -> str:
...
#     text_to_process = 'Helllo, I am John Doe!!! My email is john.doe@email.com. Visit our website www.johndoe.com <h1></h1>'
